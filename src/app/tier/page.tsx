...
import { categoryData, db } from '../../data/PKDB';
...
export default function Tiermaker() {
  ...
  const sorter = () => {
    ...
    for (let i = 0; i < tierList.length; i++) {
      ...
      childSorted = childSorted.sort((x: any, y: any) => x.id - y.id);
  ...
  const exportElementAsPNG2 = () => {
    (document.querySelector('#top') as any).scrollIntoView();
    if (coverRef.current && realDownloadRef.current) {
      if (elementRef.current) {
        (coverRef.current as any).style.height = coverHeight + 'px';
        (coverRef.current as any).style.display = 'flex';
        (realDownloadRef.current as any).style.display = 'block';
        let saveDisplay = '';
        let saveBorder = '';
        let saveWidth = '';
        var matches = document.getElementsByClassName('toggle');
        for (let i = 0; i < matches.length; i++) {
          (matches[i] as any).style.visibility = 'hidden';
        }
        let mob1 = document.getElementsByClassName('forMob1');
        let mob2 = document.getElementsByClassName('forMob2');
        (saveDisplay = mob1[0] as any).style.display;
        (saveBorder = mob1[0] as any).style.border;
        (saveWidth = mob2[0] as any).style.width;
        for (let i = 0; i < mob1.length; i++) {
          (mob1[i] as any).style.display = 'flex';
          (mob1[i] as any).style.border = '1px black solid';
          (mob2[i] as any).style.width = '120px';
        }
        let changer = document.getElementsByClassName('nameLine');
        for (let i = 0; i < changer.length; i++) {
          let target: any = changer[i];
          target.style.height = '1px';
          target.style.height = target.scrollHeight + 'px';
        }
        (coverRef.current as any).style.width = '100vw';
        (elementRef.current as any).style.width = '1450px';
        toPng(elementRef.current).then((image2) => {
          let realDown: any = realDownloadRef.current;
          realDown.children[0].src = image2;
          realDown.children[0].width = 800;
          realDown.children[0].addEventListener(
            'load',
            () => {
              // setTimeout(() => {
              toPng(realDown.children[0]).then((image3) => {
                let canvas = document.createElement('canvas');
                let img1 = new Image();
                img1.src = image3;
                img1.onload = () => {
                  canvas.width = realDown.children[0].width;
                  canvas.height = realDown.children[0].height;
                  (canvas as any)
                    .getContext('2d')
                    .drawImage(
                      img1,
                      0,
                      0,
                      realDown.children[0].width,
                      realDown.children[0].height,
                    );
                  ...
                  a.click();
                  (elementRef.current as any).style.width = '90vw';
                  for (let i = 0; i < matches.length; i++) {
                    (matches[i] as any).style.visibility = 'visible';
                  }
                  for (let i = 0; i < mob1.length; i++) {
                    (mob1[i] as any).style.display = saveDisplay;
                    (mob1[i] as any).style.border = saveBorder;
                    (mob2[i] as any).style.width = saveWidth;
                  }
                  realDown.style.display = 'none';
                  realDown.children[0].src = '';
                  (coverRef.current as any).style.width = 'auto';
                  (coverRef.current as any).style.display = 'none';
                  let changer = document.getElementsByClassName('nameLine');
                  for (let i = 0; i < changer.length; i++) {
                    let target: any = changer[i];
                    target.style.height = '1px';
  ...
  const back = (id: any) => {
    ...
    for (let i = 0; i < set.length; i++) {
      ...
      for (let j = 0; j < set[i].contentArr.length; j++) {
        if ((set[i].contentArr[j] as any).id !== selectedItem.id) {
  ...
  const backMobile = (id: any) => {
    ...
    for (let i = 0; i < copy.length; i++) {
      for (let j = 0; j < copy[i].contentArr.length; j++) {
        if ((copy[i].contentArr[j] as any).id == selectedItem.id) {
    ...
    for (let i = 0; i < newData.length; i++) {
      ...
      if (newData[i].id == e.target.id) {
        for (let j = 0; j < newData[i].contentArr.length; j++) {
          // 같은 티어내에서 빈공간에 이동 처리
          if ((newData[i].contentArr[j] as any).id == selectedItem.id) {
          ...
          for (let j = 0; j < newData[i].contentArr.length; j++) {
            ...
            if ((newData[i].contentArr[j] as any).id == selectedItem.id) {
              ...
              for (let a = 0; a < cloner.length; a++) {
                if ((cloner[a] as any).id == e.target.id) changer1 = a;
                else if ((cloner[a] as any).id == selectedItem.id) changer2 = a;
              }
  ...
  const dropMobile = (e: any) => {
    try {
      ...
      const body: any = document.getElementsByClassName('App')[0];
      body.style.overflow = 'auto';
      if (threadRef.current) {
        (threadRef.current as any).style.overflowX = 'auto';
      }
            ...
            if (e.parentNode.id == newData[i].id) {
              ...
              for (let j = 0; j < newData[i].contentArr.length; j++) {
                ...
                if ((newData[i].contentArr[j] as any).id == selectedItem.id) {
                  ...
                  let changer2 = -1;
                  for (let a = 0; a < cloner.length; a++) {
                    if ((cloner[a] as any).id == e.id) changer1 = a;
                    else if ((cloner[a] as any).id == selectedItem.id)
                      changer2 = a;
                  }
                  if (changer1 !== -1 && changer2 !== -1) {
                    let theTarget = cloner[changer2];
  ...
  const generationChanger = (e: any) => {
    let copy = dbSet.slice();
    let renew = [];
    let used = [];
    for (let i = 0; i < tierList.length; i++) {
      for (let j = 0; j < tierList[i].contentArr.length; j++) {
        used.push((tierList[i].contentArr[j] as any).name);
      }
    }
    if (e.value == 'All') {
  ...
  const move = (id: any, direction: string) => {
    let copy = tierList.slice();
    for (let i = 0; i < copy.length; i++) {
      if (copy[i].id == id) {
        let temp = copy[i];
        if (direction == 'up') {
          if (i == 0) return;
          let another = i - 1;
          let target: any = document.getElementById(`${i}:Line`);
          let target2: any = document.getElementById(`${another}:Line`);
  ...
  const dbloader = () => {
    if (localStorage.getItem('namedb')) {
      let tierDatas = JSON.parse(localStorage.getItem('namedb') as string);
      let record: any = [];
      for (let i = 0; i < tierDatas.length; i++) {
        let check = JSON.parse(localStorage.getItem(tierDatas[i]) as string);
  ...
  const loadData = (e: any) => {
    let test: any = localStorage.getItem(e.value);
  ...
  const selectGuideLang = (e: any, lang: string) => {
    e.preventDefault();
    e.stopPropagation();
    setGuideLang(lang);
  };
  ...
  const downloadTXT = (e: any) => {
    let file: any = localStorage.getItem(e.value);
  ...
  useEffect(() => {
    if (isMounted.current) {
      (belowRef.current as any).scrollIntoView();
    } else {
  ...
  useEffect(() => {
    if (threadRef.current) {
      (threadRef.current as any).scrollTo(0, 0);
    }
  }, [goleft]);

  useEffect(() => {
    if (isMounted2.current) {
      let target: any = document.getElementById(
  ...
  useEffect(() => {
    if (elementRef.current) {
      ...
      setCoverHeight((elementRef.current as any).offsetHeight);
      nameInputSizer();
    }
  }, [tierList]);
          ...
          >
            ...
            {question && (
              ...
              >
                ...
                <$GuideEachLine>
                  <RiFileSearchLine style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean' ? '데이터 불러오기' : 'Load data'}
                  </span>
                </$GuideEachLine>
                <$GuideEachLine>
                  <TbHttpDelete style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean' ? '데이터 삭제' : 'Delete data'}
                  </span>
                </$GuideEachLine>
                <$GuideEachLine>
                  <GrDocumentTxt style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean'
                      ? '티어표 데이터 txt 다운로드'
                ...
                <$GuideEachLine>
                  <GrDocumentUpload style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean'
                      ? '티어표 데이터 txt 불러오기'
                ...
                <$GuideEachLine>
                  <FaSortNumericDown style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean'
                      ? '도감번호순 정렬'
                      : 'Sort by dex order'}
                ...
                <$GuideEachLine>
                  <AiOutlineOrderedList style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean'
                      ? '세대별 분류'
                      : 'Choose generation'}
                ...
                <$GuideEachLine>
                  <IoDownloadOutline style={{ color: 'black' }} /> :{' '}
                  <span>
                    {guideLang == 'korean'
                      ? '티어표 이미지 다운로드'
                      : 'Download Tierlist'}
                        ...
                        return (
                          <$Img
                            ...
                            onDragStart={(e) => {
                              ...
                              setPreviousPlace(
                                (e.currentTarget.parentNode as any).id,
                              );
                            }}
                            onTouchStart={(e) => {
                              setSelectedItem({
                  ...
                  return (
                    <div style={{ width: '128px' }}>
                      ...
                      <$Img
                        ...
                        onTouchMove={(e) => {
                          if (threadRef.current) {
                            (threadRef.current as any).style.overflowX =
                              'hidden';
                          }
                          // const body = document.getElementsByTagName('body')[0];
                          const body: any =
